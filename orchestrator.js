...
async function main() {
  ...
  const errorPrompt = "The previous action failed. Analyze the error message in the history. If the error is 'Unable to locate package', you MUST use 'search_web'. For the query, use the error message. For the topic, use 'Ubuntu package installation'. For the subTopic, use the name of the package that failed. Analyze the search results to find a solution. Do not try the same package name again unless your research indicates it will work. For other errors, decide on a step to fix the problem. Once you have a fix, you may continue with the original goal.";

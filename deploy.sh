...
$AGENT_USER ALL=(ALL) NOPASSWD: /usr/bin/apt-get update, /usr/bin/apt-get -y upgrade, /usr/bin/apt-get -y install, /usr/bin/apt-get -y remove, /usr/bin/systemctl enable, /usr/bin/systemctl start, /usr/bin/systemctl stop, /usr/bin/systemctl restart, /usr/sbin/ufw allow, /usr/sbin/ufw deny, /usr/sbin/useradd, /usr/sbin/usermod, /usr/sbin/userdel
EOF
